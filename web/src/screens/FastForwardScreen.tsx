...
import type { FastForward, UiStrings } from "../engine/types";
...
export function FastForwardScreen(props: {
  ui: UiStrings; game: number; team: string; onDone: () => void;
}) {
  const { ui, game, team, onDone } = props;
  ...
  if (!ff) return <div className="spinner">{ui.warming_up}</div>;

  const done = shown >= ff.eliminations.length;

  return (
    <>
      <h2>{team} — {ui.dust_settles}</h2>
      <div className="panel">
        {ff.eliminations.slice(0, shown).map((e, i) => (
          <div key={i} className="ff-step">
            <span>💥 {e.name} {ui.eliminated_word}</span>
            <b>{e.alive_after} {ui.remaining_word}</b>
          </div>
        ))}
      </div>
